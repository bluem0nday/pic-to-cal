...
**Repertory venues: a dated run page outranks an undated title page (2026-08-05, Test 17).** The ranking above assumes a per-event page is about one event. At a venue that replays its catalog — a repertory cinema, a theater in rep, a recurring club night — the per-title page is *evergreen*: one permanent page per film, carrying whichever run was published to it last. Spectacle's SPLIT page still listed a February run and its SHEILA AND THE BRAINSTEM page a June one while both films were playing that August. Filing from those pages would have produced six confidently wrong dates under a ✓ badge, which is the worst output this skill can make: a hold that looks checked and sends the user on the wrong night.

So at any venue that re-runs its catalog, the dated series or run page wins over the undated per-title page, even though the title page looks more specific. **The staleness test is one question: do the page's dates fall in the month you're filing?** If a title page's showtimes sit in a different month than the source image or the series listing, that page is stale — read it for the synopsis and credits, take the schedule from the run page, and never take a date from it. And don't build per-title URLs by slugifying the name: guessed slugs mostly 404 (`/the-scenic-route/` and `/surfer-teen-confronts-fear/` both did; the real one was `/surfer/`), and a guess that happens to resolve lands you on exactly the evergreen page this rule is about.
...
| Page rendering (browser) | Blocked and login-walled pages are unreadable. Enrichment is lost — price, captions, handles, FAQ detail. Verification against an unblocked page is unaffected. **Exact-copy Details is also lost** on any page whose text the fetcher summarizes or refuses (2026-08-05): the body falls back to the credits and whatever short exact quote survived, plus the 📄 shortened-description line. Dates, times, and place are unaffected, so the hold stays correct — it just carries less of the venue's own voice. |
