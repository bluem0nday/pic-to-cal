...
Take an attached event image, find the official registration URL, and create a HOLD on the dedicated "Pic to Cal Events" Google Calendar so they don't lose track of it. The user types this skill name as either `pic-to-cal` or `pic to cal` — both are correct.
...
Follow these nine steps in order. One question at a time. Never batch questions to the user — that violates the user's working style and will frustrate them.
...
If the source gives a start but no end time and no runtime to back one out of, default the end to **2 hours** after the start, and add a line in the body — `No end time or duration found` — so the user knows the end is a placeholder they can adjust. It's a HOLD; an approximate block is fine.

If the source gives **no time at all — only a date**, don't invent a start time. File the HOLD as an **all-day** event: the thin bar across the top of the day, not a block that reserves 9-to-5 or any other span. Use the connector's `allDay: true`. Add a prominent line in the body — `⏰ No starting time found — filed as all-day banner` — so the user knows why it's a banner and that the time is the thing to look up when they click through. (Timezone doesn't apply to an all-day event, so the venue-TZ logic can be skipped in this case.)
...
If neither search attempt returned a usable URL, or if the page wouldn't load, continue to step 7 with no URL. The HOLD will get a "no source found" flag in the description. Don't ask the user whether to proceed — the BACKLOG decision is auto-skip after two tries.
...
- **calendarId**: `[the Event Holds calendar ID]` — the user's dedicated **"Pic to Cal Events"** calendar (created 2026-07-04). If a create against this ID fails, verify with `list_calendars` that a calendar named "Pic to Cal Events" still exists; if it's gone, stop and tell the user. Never fall back to `primary` — the dedicated calendar is what lets the user toggle all HOLDs on/off at once
- **summary**: `📌 Hold: [event title]` — always this exact prefix, even when the event title contains its own colon ("📌 Hold: Twin Peaks: Fire Walk With Me…" is correct; the user confirmed 2026-07-04, don't restructure to avoid the double colon)
