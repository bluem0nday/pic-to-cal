...
Take an attached event image, find the official registration URL, and create a HOLD on the user's primary Google Calendar so they don't lose track of it. The user types this skill name as either `pic-to-cal` or `pic to cal` — both are correct.

## Why this skill exists

The user screenshots events on Instagram, X, email, and flyers all the time, then forgets about them. Manual flow is: open calendar, type title, find the registration page, paste URL, set time, save. This skill collapses that to: attach image, type a trigger phrase, confirm once, done.
...
If the source gives **no time at all — only a date**, don't invent a start time. File the HOLD as an **all-day** event: the thin bar across the top of the day, not a block that reserves 9-to-5 or any other span. Use the connector's `allDay: true`. Add a line in the body — `No time listed — filed as all-day` — so the user knows why it's a banner. (Timezone doesn't apply to an all-day event, so the venue-TZ logic can be skipped in this case.)
...
- **⚠ Unverified** — a link exists but it didn't confirm the facts: a venue homepage, a monthly schedule, or a login-walled page that can't be read. List the best link so the user knows where to look, but label it so they don't trust it blindly.
...
That's it. Don't summarize what was done. Don't list the description fields back. The user can open the calendar event if they want to see the details.
