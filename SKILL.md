...
4. **Then check the residue, because the recreated file is not empty.** It carries three records the runtime re-seeds: the last user prompt (truncated), the session's title, and an AI-written one-line summary of the session. The user's own triggering message is the single thing most likely to name the event, so the new file has to be checked for the event's title, venue, and URL. **Run that check without saying the words (2026-08-07).** The search command is itself recorded in the file being searched, so naming the event on the command line writes the name straight back into the one file the run just cleaned. Put the terms in a scratch file outside the transcript directory, point the search at that file, and delete the scratch file when the check is done. Report a hit rather than quietly deleting again — a second delete re-seeds the same three records. Where the runtime can rename a session, set a neutral title *before* the scrub so the two title records have nothing to carry.
...
On a private run the Title line shows the title that will actually be filed, 🤫 included: `Title:       📌 🤫 Hold: [event title]`. Same rule as every other field in the block — real values, never the template.
...
The label is the calendar title exactly as filed, so on a private run it carries the 🤫 too: `📌 🤫 Hold: [event title] — [Mon D, time]`.
